...
function connect() {
  ...
  bot = mineflayer.createBot({
    ...
    // Mineflayer's protocol support, which causes malformed movement packets
    // and "invalid_player_movement" kicks. Servers running ViaVersion (like
    // this one) translate an older client version up to the real server
    // version automatically, so this sidesteps the problem entirely.
    version: settings.minecraftVersion || false,
  ...
  bot.once('spawn', () => {
    ...
    // Grace period: stay completely still for 1 minute right after spawning.
    // Nearby chunks may not be fully loaded yet, and the bot's local physics
    // engine starts predicting gravity/position immediately - if that races
    // ahead of the server's own chunk/terrain state, the resulting position
    // jump can trigger an invalid-movement kick. Waiting first avoids that,
    // and also helps confirm whether movement code is even the cause.
    log('Standing still for 60s grace period before starting movement')
    setTimeout(() => {
      if (!bot || status !== 'online') return
      log('Grace period over - starting movement')
      startMovementLoops(defaultMove, m)
    }, 60000)
